 ...
 */
public class MaxHeap<T> implements ExtrinsicPQ<T> {
    ...
    public MaxHeap() {
        contents = new MaxHeap.Node[CAPACITY];

        /* Add a dummy item at the front of the MinPQ.ArrayHeap so that the getLeft,
    ...
    /**
     * Returns the index of the node with larger priority. Precondition: not
     * both nodes are null.
     */
    ...
    private void swim(int index) {
        ...
        if (inBounds(parentIndex) && max(index, parentIndex) == index) {
    ...
    public T remove() {
        T maxItem = peek();
        contents[MAX_INDEX] = getNode(size);
        sink(MAX_INDEX);
        contents[size--] = null;
        return maxItem;
    }
    ...
    private void resize(int capacity) {
        Node[] temp = new MaxHeap.Node[capacity];
