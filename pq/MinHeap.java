 ...
 */
public class MinHeap<T> implements ExtrinsicPQ<T> {
    ...
    public MinHeap() {
        contents = new MinHeap.Node[CAPACITY];

        /* Add a dummy item at the front of the MinPQ.MinHeap so that the getLeft,
    ...
    private void swim(int index) {
        ...
        if (inBounds(parentIndex) && min(index, parentIndex) == index) {
    ...
    private void sink(int index) {
        ...
        int changeIndex = min(leftIndex, rightIndex);
        if (inBounds(changeIndex) && min(index, changeIndex) == changeIndex) {
    ...
    @Override
    public T peek() {
        Node root = getNode(MIN_INDEX);
        return Objects.isNull(root) ? null : root.item();
    }
    ...
    public T remove() {
        T minItem = peek();
        contents[MIN_INDEX] = getNode(size);
        sink(MIN_INDEX);
        contents[size--] = null;
        return minItem;
    }
    ...
    public void changePriority(T item, double priority) {
        for (int i = MIN_INDEX; i < size; i++) {
            if (contents[i].item().equals(item)) {
                contents[i].setPriority(priority);
            }
        }
    }
    ...
    private void resize(int capacity) {
        Node[] temp = new MinHeap.Node[capacity];
